...
class AccountUpdateRequest(BaseModel):
    ...
    status: Literal["正常", "限流", "存疑", "异常", "禁用"] | None = None
    quota: int | None = None
    proxy: str | None = None
...
def _account_status_label(
        category: Literal["normal", "limited", "suspicious", "abnormal", "disabled"],
) -> str:
    return {
        "normal": "正常",
        "limited": "限流",
...
def _status_matches_filter(account: dict[str, Any], status_filter: str) -> bool:
    ...
    if not status_filter or status_filter == "all":
        return True
    if status_filter in {"normal", "limited", "suspicious", "abnormal", "disabled"}:
