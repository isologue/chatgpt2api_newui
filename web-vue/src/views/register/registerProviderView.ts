...
export const remailSupplyOptions = [
  { value: 'private_first', label: '私库优先' },
  { value: 'public_only', label: '仅公库' },
] as const
...
export const providerTypeKeys: Record<string, string[]> = {
  ...
  remail: ['api_base', 'api_key', 'project_id', 'email_suffix', 'supply'],
...
export function defaultProvider(type = 'cloudmail_gen'): RegisterProvider {
  ...
  switch (type) {
    ...
    case 'remail':
      return { ...base, api_base: '', api_key: '', project_id: '', email_suffix: '', supply: 'private_first' }
    case 'donemail':
...
export function normalizeRegisterConfig(raw: LegacyRegisterConfig): LegacyRegisterConfig {
  ...
  return {
    ...
    ...raw,
    mail,
    clearance: { ...defaultRegisterConfig.clearance, ...(raw.clearance || {}) },
...
export function legacyRegisterPayload(config: LegacyRegisterConfig): Partial<LegacyRegisterConfig> {
  return {
    ...
    clearance: {
      ...config.clearance,
      enabled: config.clearance.enabled === true,
      mode: config.clearance.mode || 'none',
      timeout_sec: Math.max(1, Number(config.clearance.timeout_sec) || 60),
      refresh_interval: Math.max(60, Number(config.clearance.refresh_interval) || 3600),
    },
...
export function providerRequirementMessages(provider: RegisterProvider) {
  ...
  switch (type) {
    ...
    case 'gptmail':
      ...
      break
    case 'remail':
      requireValue(provider.api_base, 'Remail API URL')
      requireValue(provider.api_key, 'API Token')
      requireValue(provider.project_id, '项目')
      requireValue(provider.email_suffix, '邮箱后缀')
      break
    case 'donemail':
...
export function providerUsesApiBase(provider: RegisterProvider) {
  return ['cloudmail_gen', 'cloudflare_temp_email', 'moemail', 'inbucket', 'yyds_mail', 'ddg_mail', 'donemail', 'remail'].includes(providerType(provider))
}

export function providerUsesApiKey(provider: RegisterProvider) {
  return ['tempmail_lol', 'moemail', 'duckmail', 'gptmail', 'yyds_mail', 'remail'].includes(providerType(provider))
}
...
export function apiBaseLabel(provider: RegisterProvider) {
  ...
  if (type === 'remail') return 'Remail API URL'
  return 'API Base'
}
...
export function apiBasePlaceholder(provider: RegisterProvider) {
  ...
  if (type === 'remail') return 'https://remail.example.com'
  return ''
}
